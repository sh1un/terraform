...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == "GET" and path == dishes_path:
            ...
            body = json.loads(event["body"])
            response = update_dish(
                body["dish_id"], body["update_key"], body["update_value"]
            )
...
def save_dish(item):
    try:
        ...
        body = {"Operation": "SAVE", "Message": "SUCCESS", "Item": item}
...
def update_dish(dish_id, update_key, update_value):
    try:
        ...
        body = {"Operation": "UPDATE", "Message": "SUCCESS", "Item": response}
...
def delete_dish(dish_id):
    try:
        response = table.delete_item(Key={"dish_id": dish_id}, ReturnValues="ALL_OLD")
